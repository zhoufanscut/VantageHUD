...
- `statusline.sh` (POSIX, no bashisms) is a **caching wrapper**, not just a launcher. Hot path = `cat` the last rendered line for this session and kick a background Node refresh for the *next* frame. First render per session is **synchronous** because Claude Code v2.1.x doesn't re-poll until user interaction. All runtime files — the shell's per-session render cache + `mkdir`-based render lock, *and* the Node HUD's state — live flat in `cache/` (gitignored), each named `<name>.<session>.json`. Keep it relocatable: the cache dir is derived from the script dir (shell) / module location (Node), honors `HUD_CACHE_DIR`, and is never hardcoded.
...
- **State is centralized, never written into the user's project.** All runtime files live flat in one cache dir — by default the HUD install's own `cache/` (derived from the module's location in `getCacheDir`, so it follows a relocated install); `HUD_CACHE_DIR` overrides it and the shell wrapper honors the same var, so the two layers never diverge. Every file is `<name>.<session>.json` (`hud-stdin-cache.<session>.json`, `hud-state.<session>.json`, `compact-requested.<session>.json`); the globally-unique session id keeps unrelated sessions **and** projects from colliding, so there is no per-project subdirectory (and the stdin/stabilization cache can no longer be clobbered across sessions). Always resolve via `src/lib/worktree-paths.js` (`getCacheDir`, `sessionCacheFile`, `ensureCacheDir`); never hardcode. The session key is derived once in `index.js#resolveSessionKey` (stdin `session_id` → session-id env vars → transcript UUID → `default`) and threaded into every cache read/write.
