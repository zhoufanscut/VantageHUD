...
- **State location is not always `.claude-statusline/`.** When `HUD_STATE_DIR` is set it switches to a centralized `$HUD_STATE_DIR/<project-id>` path. Always resolve via `src/lib/worktree-paths.js` (`getStateRoot`, `resolveSessionStatePath`); never hardcode. `validateWorkingDirectory()` always returns the git **worktree root**, never a subdirectory, and paths are validated against `..`/absolute traversal.
